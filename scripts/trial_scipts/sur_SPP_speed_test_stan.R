...
functions {
  ...
  matrix ob_knot_C(matrix ob_knot_dm, real dd_spp, real sigma_spp){
    ...
    C_knot_ob = sigma_spp * exp(-dd_spp * ob_knot_dist);
  ...
  vector interp_back(vector spp, matrix C_inv, matrix C_knot_ob, int N_obs, 
    real dd_spp, real sigma_spp){
  
    vector[N_obs] SPP_int;
    
    return SPP_int = C_knot_ob * C_inv * spp;  
  
  }
  
  vector inter_weight_ave(vector spp, matrix knot_ob_dm, matrix knot_ob_ind, int N_obs, int N_nn, 
    real dd_spp){
...
transformed parameters { 
  ...
  interp_back(spp, C_spp_inv, C_spp_s, N, dd_spp, sigma_spp);
...
SPP_sur_stan <- stan(model_code = stan_model2, data = stan_dat, iter = 200, warmup = 150, 
  cores = 2, chains = 2, control = list(adapt_delta = 0.8), include = TRUE, 
...
setwd(code_loc)
stan_funs = expose_stan_functions('SPP_speedtest.stan')

# set up some stuff for testing 
library(MASS)
knot_cm = cov_var_maker(dm_knots, dim(dm_knots)[1], 1.5, 0.5)
spp = mvrnorm(1, rep(0, dim(dm_knots)[1]), knot_cm)
knot_ob_dist_nn = matrix(NA, nrow = dim(dm_knots_obs)[1], ncol = 10)
nn_inds = list()
for(i in 1:dim(dm_knots_obs)[1]){
  
  knot_ob_dist_nn[i, ] = sort(dm_knots_obs[i, ], FALSE)[1:10]
  nn_inds[[i]] = which(dm_knots_obs[i, ] %in% knot_ob_dist_nn[i, ])

}


microbenchmark(cov_var_maker(dm_knots, dim(dm_knots)[1], 1.5, 0.5),
  ob_knot_C(dm_knots_obs, 1.5, 0.5),
  interp_back(spp, knot_cm, dm_knots_obs, dim(dm_knots_obs)[1]),
  inter_weight_ave(spp, dm_knots_obs, dim(dm_knots_obs)[1], dim(dm_knots)[1], 1.5),
  inter_NK(spp, knot_ob_dist_nn, nn_inds, 10, dim(dm_knots_obs)[1], 1.5))
